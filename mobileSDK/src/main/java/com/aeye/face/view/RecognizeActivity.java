...
import com.aeye.face.uitls.FaceNoticeDialog;
...
public class RecognizeActivity extends Activity implements
    ...
    /** 首次无人脸 / 后续出框 / 多人脸：连续达到该时长后展示「验证失败」，页面同步显示 20s 倒计时 */
    private static final long NO_FACE_FAIL_DELAY_MS = 20000L;
    private static final int NO_FACE_FAIL_SEC = 20;
    ...
    /** 动作 10s / 出框 20s 倒计时 */
    private FaceCountdownController mCountdown;
    /** 返回按钮、倒计时数字图片、语音开关图片 **/
    ...
    /** 避免失败页重试与 surfaceCreated 并发 Camera.open */
    private volatile boolean mCameraOpenInFlight;
    /** 摄像头方向 (前置/后置) **/
    ...
    /** USB 调试拦截：跳过相机初始化，确认后结束流程 */
    private boolean mUsbDebugBlocked;
    /** 保持屏幕唤醒的锁 **/
    ...
    int textId = -1;
    /** 已播过口令的动作 ID；出框再回框不重播，与纯动作一致。换动作时更新。 */
    private int mPlayedPoseVoiceId = Integer.MIN_VALUE;
    /** 出框/过远语音延迟播放，避免检测抖动时和动作口令打架 */
    private Runnable mQualityVoiceRunnable;
    ...
    /** 扫描环动画（蓝弧旋转 / 成功填充），绘制在 {@link ScanRingOverlayView} */
    private ScanRingAnimator mScanRing;
    /** 是否启用扫描环圆外白底遮罩（竖屏白底模式） */
    private boolean mRingHoleUiEnabled;
    /** 定格最后一帧预览 **/
    private ImageView ivPreviewFreeze;
    /** 提交核验：冻帧圆形遮罩 + loading */
    private View faceSubmitOverlay;
    private ProgressBar pbSubmitLoading;
    ...
    /** 活体动作已完成，正在提交 / 轮询核验结果；此期间禁止返回 */
    private boolean mFaceVerifying;
    /** 核验成功后倒计时秒数，到 0 关闭页面 */
    private static final int SUCCESS_AUTO_FINISH_SEC = 3;
    ...
    /** 从后台回到取景：倒计时需从头开始，跳过 3-2-1 准备动画 */
    private boolean mResumeFromBackground;
    /** 进后台时正处于动作阶段（脸在框内），回来后继续当前动作而不是回到「正视摄像头」 */
    private boolean mPausedWhileInAction;
    /** 预览刚打开时曝光未稳，这段时间内不提示过暗/过亮 */
    private static final long LIGHT_CHECK_GRACE_MS = 2000L;
    /** 未开始预览前不允许环境光提示；startRecog 后为 now+grace */
    private long mLightCheckAfterElapsedMs = Long.MAX_VALUE;
    private long mNoFaceSinceElapsedMs = -1L;
    private Runnable mNoFaceFailRunnable;
    /** 多人脸等待起始时间；解码线程会读，需可见跨线程 */
    private volatile long mMultiFaceSinceElapsedMs = -1L;
    private volatile boolean mWaitingMultiFace;
    /** 环境光过暗/过亮等待中 */
    private volatile boolean mWaitingLightAbnormal;
    /** {@link AEFaceQuality#QUALITY_DARK} / {@link AEFaceQuality#QUALITY_BRIGHT}，0 表示无 */
    private volatile int mLightAbnormalQuality;
    private volatile long mLightAbnormalSinceElapsedMs = -1L;
    /** 是否已上报二维码终态（4 未通过 / 2 异常退出）；提交成功只置位、不上报 status=5 */
    private boolean mQrRecordFinalized;
    /** 本会话动作活体未通过次数；满 3 次上报 status=4 + failedType=1 */
    private int mLivenessFailCount;
    /** 「重新核验」点击次数；超过 3 次弹出温馨提示，不再重试 */
    private static final int MAX_FAIL_RETRY_CLICKS = 3;
    private int mFailRetryClickCount;
    ...
    /** 0 未知 / 1 在圆框内 / -1 出圆框，避免每帧重置 10 秒/20 秒 */
    private int mGuideOvalState = 0;
    /** 页面已展示的入/出框态，滞后于 {@link #mGuideOvalState}，用于防抖 */
    private int mDisplayedGuideState = 0;
    private Runnable mGuideUiRunnable;
    /** 已排队的防抖方向：true 入框 / false 出框 */
    private Boolean mPendingGuideUiIn;
    private String mPendingOutHintKey;
    /** 出框文案实际展示时间，避免「靠近」与「移入框内」每帧对打 */
    private long mOutHintAppliedAt;
    private static final long OUT_HINT_SWITCH_MS = 800L;
    private static final long GUIDE_LEAVE_UI_DELAY_MS = 700L;
    private static final long GUIDE_ENTER_UI_DELAY_MS = 500L;
    /** 过远需稳定后再切，避免与动作 10s 来回跳 */
    private static final long FAR_UI_HOLD_MS = 350L;
    private Runnable mPendingFarUiRunnable;
    ...
    public boolean isSilentAliveMode() {
        return aliveMode == AEFaceParam.ALIVEMODE_SILENT;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (UsbDeveloperModeGuard.shouldBlock(this)) {
            mUsbDebugBlocked = true;
            mFinish = true;
            UsbDeveloperModeGuard.showBlockDialogAndExit(this, () ->
                    finishActivityByOther(AEFacePack.ERROR_DANGER_DEVICE,
                            getString(R.string.aeye_usb_debug_block_message)));
            return;
        }
        if (DeviceRootGuard.shouldBlock(this)) {
            mUsbDebugBlocked = true;
            mFinish = true;
            DeviceRootGuard.showBlockDialogAndExit(this, () ->
                    finishActivityByOther(AEFacePack.ERROR_DANGER_DEVICE,
                            getString(R.string.face_fail_device_unsafe_detail)));
            return;
        }
...
//        }
        ...
        /** 认证倒计时 **/
        mCountdown = new FaceCountdownController(
                findViewById(R.id.tvRecogTimeCountdown),
                findViewById(R.id.tvActionCountdown),
                new CountdownHost());
        mCountdown.hide();

        mScanRing = new ScanRingAnimator(findViewById(R.id.scan_ring_overlay), this::isScanRingBlocked);
        ivPreviewFreeze = findViewById(R.id.iv_preview_freeze);
        faceSubmitOverlay = findViewById(R.id.face_submit_overlay);
        pbSubmitLoading = findViewById(R.id.pb_submit_loading);
        applySubmitOverlayCircleClip();
        mRingHoleUiEnabled = AEFacePack.getInstance().isWhiteBackgroud()
                && !AEFacePack.getInstance().isLand();
        mScanRing.setHoleMaskEnabled(mRingHoleUiEnabled);
        ...
        if (btnFailOther != null) {
            btnFailOther.setOnClickListener(v -> handleUserExit());
        }
...
//        }
        ...
        mCountdown.setLegacyVisible(false);
        initVoice();
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                ...
                case UI_MSG_HINT_TEXT_SHOW:
                    ...
                    if ("aeye_camera_notice".equals(text)) {
                        break;
                    }
                    if ("aeye_quality_out".equals(text) || "quality_out".equals(text)
                            || "face_far".equals(text)) {
                        hintColorKind = HINT_COLOR_THEME;
                        if (mWaitingMultiFace || mWaitingLightAbnormal) {
                            break;
                        }
                        if (shouldHoldOutOfFrameHint()) {
                            mPendingOutHintKey = text;
                            break;
                        }
                        if (shouldHoldOutHintSwitch(text)) {
                            mPendingOutHintKey = text;
                            break;
                        }
                    }
                    if (text != null && text.equals(mDisplayedCheckHintKey)
                        ...
                        int color = ContextCompat.getColor(RecognizeActivity.this,
                    ...
                    switch (text) {
                        ...
                        case "quality_out":
                            ...
                            mOutHintAppliedAt = SystemClock.elapsedRealtime();
                            break;
                        case "face_far":
                        ...
                        case "keep_face":
                            ...
                            break;
                        case "more_people":
                            shownResId = R.string.aeye_notice_morepeople;
                            tvCheckHint.setText(shownResId);
                            break;
                        case "light_dark":
                            shownResId = R.string.aeye_quality_dim;
                            tvCheckHint.setText(shownResId);
                            break;
                        case "light_bright":
                            shownResId = R.string.aeye_quality_bright;
                            tvCheckHint.setText(shownResId);
                            break;
                    }
                ...
                case UI_MSG_MESSAGE_BOX:
                    ...
                    showInPlaceFailUi(false, resolveLoseFaceFailDetail(), resolveLoseFaceFailCode());
                    break;
                case UI_MSG_TIMEOUT_BOX:
                    noteLivenessFailForQrRecord();
                    showInPlaceFailUi(false, getString(R.string.face_fail_action_no_face_detail),
                            FaceUniResultCodes.RESULT_ACTION_TIMEOUT);
                    break;
...
//                    }
    ...
    public void showAlivePose(int id, boolean voice, boolean anim) {
        ...
        if (voice && voiceTriggle && audioId != 0) {
    ...
    public void restartTimer(long timeout) {
        restartTimer(timeout, FaceCountdownController.ACTION);
    }

    private void restartTimer(long timeout, int kind) {
        if (mCountdown == null || !mCountdown.restart(timeout, kind)) {
            return;
        }
        if (kind == FaceCountdownController.ACTION && handler != null
                && AEFacePack.getInstance().isAliveOff()
    ...
    /** 纯动作、动作+炫彩的动作阶段才展示 10 秒入框倒计时 */
    private boolean shouldShowActionCountdown() {
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return false;
        }
        if (aliveMode == AEFaceParam.ALIVEMODE_MOTION_LIGHT) {
            return !mLightFlashStarted;
        }
        return aliveMode == AEFaceParam.ALIVEMODE_MOTION
                && !AEFacePack.getInstance().isAliveOff();
    }

    /** 动作 / 炫彩 / 动作+炫彩 / 静默：脸不在框内连续 20 秒则验证失败 */
    private boolean shouldUseOutOfFrameFail() {
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return false;
        }
        return canShowCountdownNumber();
    }

    /** 动作 10s、出框 20s（含炫彩、静默）都显示同一处倒计时 */
    private boolean canShowCountdownNumber() {
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return false;
        }
        if (isSilentAliveMode()) {
            return true;
        }
        if (AEFacePack.getInstance().isAliveOff()) {
            return false;
        }
        return aliveMode == AEFaceParam.ALIVEMODE_MOTION || isLightAliveMode();
    }

    private void hideActionCountdown() {
        if (mCountdown != null) {
            mCountdown.hide();
        }
    }

    /** 人脸入框：立刻取消 20 秒并恢复动作计时；文案等稳定后再切 */
    private void onFaceEnteredGuide() {
        cancelPendingFarUi();
        mGuideOvalState = 1;
        cancelNoFaceFailTimer();
        cancelMultiFaceFailTimer();
        faceDisplayed = true;
        mPendingOutHintKey = null;
        if (shouldShowActionCountdown()) {
            if (mDisplayedGuideState != 1) {
                mCountdown.setFrozen(true);
                hideActionCountdown();
                // 出框后再入框按产品重计 10s；尚未展示入框 UI 时先不启动，避免第一眼就是 9s
                if (mDisplayedGuideState == -1 || mCountdown.getActionRemainSec() <= 0) {
                    mCountdown.setActionRemainSec((int) AEFacePack.getInstance().getMotionTime());
                }
            } else {
                mCountdown.setFrozen(false);
                int sec = mCountdown.getActionRemainSec() > 0
                        ? mCountdown.getActionRemainSec()
                        : (int) AEFacePack.getInstance().getMotionTime();
                mCountdown.clearActionRemain();
                restartTimer(sec, FaceCountdownController.ACTION);
            }
        } else {
            stopTimer();
            hideActionCountdown();
        }
        scheduleGuideUiCommit(true);
    }

    /** 人脸出框：立刻改跑 20 秒；文案/数字等稳定后再切，避免边缘抖动闪烁 */
    private void onFaceLeftGuide() {
        mGuideOvalState = -1;
        if (!shouldUseOutOfFrameFail()) {
            if (shouldShowActionCountdown() || isLightAliveMode()) {
                stopTimer();
            }
            scheduleGuideUiCommit(false);
            return;
        }
        mCountdown.stashActionRemain();
        if (mDisplayedGuideState != -1) {
            mCountdown.setFrozen(true);
            // 仍展示入框 UI 时不要立刻藏 10s，等出框提交后再切，避免临界抖动
        }
    ...
    private boolean shouldHoldOutOfFrameHint() {
        return mGuideOvalState < 0 && mDisplayedGuideState >= 0;
    }

    private boolean shouldHoldInFrameHint() {
        return mGuideOvalState > 0 && mDisplayedGuideState <= 0;
    }

    private static boolean isOutHintKey(String key) {
        return "aeye_quality_out".equals(key) || "quality_out".equals(key) || "face_far".equals(key);
    }

    private static boolean sameOutHint(String a, String b) {
        boolean aOut = "aeye_quality_out".equals(a) || "quality_out".equals(a);
        boolean bOut = "aeye_quality_out".equals(b) || "quality_out".equals(b);
        if (aOut && bOut) {
            return true;
        }
    ...
    /** 靠近一点不要挡住「移入框内」；反过来才短防抖，避免两句对打。 */
    private boolean shouldHoldOutHintSwitch(String incoming) {
        if (!"face_far".equals(incoming)) {
            return false;
        }
        if (!isOutHintKey(mDisplayedCheckHintKey) || "face_far".equals(mDisplayedCheckHintKey)) {
            return false;
        }
        if (tvCheckHint == null || tvCheckHint.getVisibility() != View.VISIBLE) {
            return false;
        }
        return SystemClock.elapsedRealtime() - mOutHintAppliedAt < OUT_HINT_SWITCH_MS;
    }

    private boolean shouldHoldOutHintRes(int incomingResId) {
        if (incomingResId != R.string.aeye_face_far) {
            return false;
        }
        if (mDisplayedCheckHintResId != R.string.aeye_quality_out) {
            return false;
        }
        if (tvCheckHint == null || tvCheckHint.getVisibility() != View.VISIBLE) {
            return false;
    ...
    /**
     * 出框/过远提示。解码线程每帧都可能回调，文案必须走滞回，不能直接刷。
     */
    public void notifyOutOfFrameHint(String key) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            final String k = key;
            runOnUiThread(() -> notifyOutOfFrameHint(k));
            return;
        }
        if (TextUtils.isEmpty(key)) {
            key = "aeye_quality_out";
        }
        mPendingOutHintKey = key;
        if (shouldHoldOutOfFrameHint()) {
            return;
        }
        if (shouldHoldOutHintSwitch(key)) {
            return;
        }
        if ("face_far".equals(key) || mDisplayedGuideState < 0 || mFaceOK < 0) {
            showHint(key, HINT_COLOR_THEME);
        }
    }

    /** 已处于出框展示时，仍要把 pending 文案刷上去（例如从「靠近一点」变成无人脸）。 */
    private void publishPendingOutHint() {
        if (mWaitingMultiFace || mWaitingLightAbnormal) {
            return;
        }
        String key = mPendingOutHintKey != null ? mPendingOutHintKey : "aeye_quality_out";
        if (sameOutHint(key, mDisplayedCheckHintKey)
                && tvCheckHint != null && tvCheckHint.getVisibility() == View.VISIBLE) {
            return;
        }
        showHint(key, HINT_COLOR_THEME);
    }

    private void cancelGuideUiCommit() {
        if (mGuideUiRunnable != null && mUIHandler != null) {
            mUIHandler.removeCallbacks(mGuideUiRunnable);
            mGuideUiRunnable = null;
        }
        mPendingGuideUiIn = null;
    }

    /**
     * 同一方向的防抖只排一次。过远/出框会每帧回调，若每次都重计 500ms，文案永远不会提交。
     * 首次进页（尚未展示过入框/出框文案）立即提交，让 20s 与「请将脸移入框内」同时出现。
     */
    private void scheduleGuideUiCommit(final boolean inOval) {
        if (mUIHandler == null) {
            commitGuideUi(inOval);
            return;
        }
        int want = inOval ? 1 : -1;
        if (mDisplayedGuideState == want) {
            mCountdown.setFrozen(false);
            if (!inOval) {
                publishPendingOutHint();
            }
            return;
        }
        if (mGuideUiRunnable != null && mPendingGuideUiIn != null && mPendingGuideUiIn == inOval) {
            return;
        }
        cancelGuideUiCommit();
        mPendingGuideUiIn = inOval;
        long delay;
        if (inOval) {
            delay = GUIDE_ENTER_UI_DELAY_MS;
        } else if (mDisplayedGuideState == 0) {
            delay = 0L;
        } else {
            delay = GUIDE_LEAVE_UI_DELAY_MS;
        }
        mGuideUiRunnable = () -> {
            mGuideUiRunnable = null;
            mPendingGuideUiIn = null;
            if (inOval != (mGuideOvalState == 1)) {
                return;
            }
            commitGuideUi(inOval);
        };
        if (delay <= 0) {
            mUIHandler.post(mGuideUiRunnable);
        } else {
            mUIHandler.postDelayed(mGuideUiRunnable, delay);
        }
    }

    private void commitGuideUi(boolean inOval) {
        mDisplayedGuideState = inOval ? 1 : -1;
        mCountdown.setFrozen(false);
        if (inOval) {
            mPendingOutHintKey = null;
            if (mDisplayedCheckHintResId == R.string.aeye_face_far) {
                mDisplayedCheckHintResId = 0;
                mDisplayedCheckHintKey = null;
            }
            if (shouldShowActionCountdown() && !mCountdown.isAction()) {
                int sec = mCountdown.getActionRemainSec() > 0
                        ? mCountdown.getActionRemainSec()
                        : (int) AEFacePack.getInstance().getMotionTime();
                mCountdown.clearActionRemain();
        ...
        publishCountdownNow();
        if (mWaitingMultiFace) {
            showHint("more_people", HINT_COLOR_THEME);
            return;
        }
        if (mWaitingLightAbnormal) {
            showHint(lightAbnormalHintKey(), HINT_COLOR_THEME);
            return;
        }
        String key = mPendingOutHintKey != null ? mPendingOutHintKey : "aeye_quality_out";
        mOutHintAppliedAt = SystemClock.elapsedRealtime();
        showHint(key, HINT_COLOR_THEME);
        if (voiceTriggle) {
            if ("face_far".equals(key)) {
                playGuideLeaveVoice(R.raw.aeye_quality_far, R.string.aeye_face_far);
            } else {
                // 首次无人脸时 showHint 可能已写入 textId，不能再走 showQualityHint 的去重，否则会静音
                playGuideLeaveVoice(R.raw.aeye_out, R.string.aeye_quality_out);
            }
        }
    }

    /**
     * 出框 / 无人脸语音。与文案是否已展示解耦：第一次进页无人脸时文案和 20s 同步出现，语音也要播。
     */
    private void playGuideLeaveVoice(final int voiceResId, final int hintResId) {
        if (!voiceTriggle || voiceResId == 0) {
            return;
        }
        if (mQualityVoiceRunnable != null) {
            mUIHandler.removeCallbacks(mQualityVoiceRunnable);
            mQualityVoiceRunnable = null;
        }
        mQualityVoiceRunnable = () -> {
            mQualityVoiceRunnable = null;
            if (!voiceTriggle || mInPlaceFailUi || mInPlaceSuccessUi) {
                return;
            }
            if (mFaceOK > 0 || mGuideOvalState == 1) {
                return;
            }
            AudioUtils.playVoice(getApplication(), voiceResId);
            textId = hintResId;
        };
        mUIHandler.postDelayed(mQualityVoiceRunnable, 250);
    }

    private void publishCountdownNow() {
        if (mCountdown != null) {
            mCountdown.publishNow();
        }
    }

    private void restoreInFrameHint() {
        ...
        int curPose = lightHandler != null ? lightHandler.getCurPos()
                : (handler != null ? handler.getCurPos() : pose);
        showAlivePose(curPose, false, false);
    }

    /**
     * 炫彩闪光阶段：脸在不在圆框只驱动 20 秒出框计时，不通过 {@link #showFaceOut(boolean)}
     * 以免暂停色光后无法恢复。
     */
    public void notifyGuideOvalForTimeout(boolean inOval) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(() -> notifyGuideOvalForTimeout(inOval));
            return;
        }
        int next = inOval ? 1 : -1;
        if (next == mGuideOvalState) {
            return;
        }
        mGuideOvalState = next;
        if (inOval) {
            onFaceEnteredGuide();
        } else {
            onFaceLeftGuide();
    ...
    protected void onResume() {
        ...
        // 提交/轮询、结果页、流程结束：不重开相机
        if (shouldKeepCameraOff()) {
            return;
        }
...
//        float rate = 1.5f;
        ...
        if (hasSurface || isSurfaceReady(surfaceHolder)) {
            hasSurface = true;
    ...
    private boolean isSurfaceReady(SurfaceHolder holder) {
        return holder != null && holder.getSurface() != null && holder.getSurface().isValid();
    }
    ...
    /** 遮罩裁成与扫描环圆孔一致，避免方块压在灰轨外。 */
    private void applySubmitOverlayCircleClip() {
        if (faceSubmitOverlay == null || Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            return;
        }
        faceSubmitOverlay.addOnLayoutChangeListener((v, l, t, r, b, ol, ot, or, ob) ->
                clipSubmitOverlayToPreviewHole());
        faceSubmitOverlay.post(this::clipSubmitOverlayToPreviewHole);
    }

    private void clipSubmitOverlayToPreviewHole() {
        if (faceSubmitOverlay == null || Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            return;
        }
        final int w = faceSubmitOverlay.getWidth();
        final int h = faceSubmitOverlay.getHeight();
        if (w <= 0 || h <= 0) {
            return;
        }
        faceSubmitOverlay.setOutlineProvider(new ViewOutlineProvider() {
            @Override
            public void getOutline(View view, Outline outline) {
                ...
                float holeR = ScanRingOverlayView.computePreviewHoleRadius(
                        view.getContext(), vw);
                int cx = vw / 2;
                int cy = vh / 2;
                int radius = Math.round(holeR);
                outline.setOval(cx - radius, cy - radius, cx + radius, cy + radius);
            }
        });
        faceSubmitOverlay.setClipToOutline(true);
    }

    private void showSubmitOverlay() {
        if (faceSubmitOverlay == null) {
            return;
        }
        faceSubmitOverlay.setVisibility(View.VISIBLE);
        if (pbSubmitLoading != null) {
            pbSubmitLoading.setVisibility(View.VISIBLE);
        }
    }

    private void hideSubmitOverlay() {
        if (pbSubmitLoading != null) {
            pbSubmitLoading.setVisibility(View.GONE);
        }
        if (faceSubmitOverlay != null) {
            faceSubmitOverlay.setVisibility(View.GONE);
        }
    }

    /**
     * 提交 / 轮询核验结果期间锁定返回：标题栏返回仍显示但不可点，系统返回键/手势不响应。
     * 否则轮询未结束就退出，后台收不到任何终态。
     */
    private void setFaceVerifying(boolean verifying) {
        mFaceVerifying = verifying;
        updateReturnButtonForVerifying();
    }

    private void updateReturnButtonForVerifying() {
        if (ivReturn == null) {
            return;
        }
        if (!AEFacePack.getInstance().isOpenReturnButton()) {
            ivReturn.setVisibility(View.GONE);
            ivReturn.setEnabled(false);
            ivReturn.setClickable(false);
            return;
        }
        boolean allow = !mFaceVerifying;
        ivReturn.setVisibility(View.VISIBLE);
        ivReturn.setEnabled(allow);
        ivReturn.setClickable(allow);
    }
    ...
    private void prepareRecog() {
        ...
        if (AEFacePack.getInstance().isShowPrepare()) {
            ...
            startRecog();
            // 进页先等检测：脸已在框内则直接做动作，不要先播「请将脸移入框内」
        }
    }
    ...
    private void prepareRecogLight() {
        ...
        startRecog();
        if (aliveMode == AEFaceParam.ALIVEMODE_MOTION_LIGHT) {
            int curPose = lightHandler != null ? lightHandler.getCurPos() : pose;
            setPose(curPose);
            // 进页先等检测：脸在框内再出口令，没脸再提示「请将脸移入框内」
        }
    }
    ...
    /**
     * @param keepProgress true：后台返回，保留当前动作/序列，只重开预览
     */
    private void startRecog(boolean keepProgress) {
        if (isLightAliveMode()) {
            startRecogLight(keepProgress);
            return;
        }
        if (!keepProgress && AEFacePack.getInstance().isAliveOff() && !isSilentAliveMode()) {
            if (AEFacePack.getInstance().isModelAllSide()) {
        ...
        // 动作 10 秒倒计时只在人脸入框后由 onFaceEnteredGuide 启动；
        // 第 15 项首次无人脸 20s 与「请将脸移入框内」在 onFaceLeftGuide 同步出现
        ...
        handler.restartPreviewAndDecode(!keepProgress);
        armLightCheckGrace();
        prepareScanRingOverlay();
    }

    private void startRecogLight() {
        startRecogLight(false);
    }

    private void startRecogLight(boolean keepProgress) {
        ...
        if (Looper.myLooper() != Looper.getMainLooper()) {
            final boolean keep = keepProgress;
            runOnUiThread(() -> startRecogLight(keep));
            return;
        }
        ...
        if (lightHandler != null) {
            lightHandler.restartPreviewAndDecode(!keepProgress);
        }
        armLightCheckGrace();
        prepareScanRingOverlay();
    }
    ...
    private void resetLightSessionFlags() {
        ...
        clearPicNumber();
        setIsFirstHasFace(false);
        mPlayedPoseVoiceId = Integer.MIN_VALUE;
        timeHandler.removeMessages(MSG_CODE_UPDATE);
        timeHandler.removeMessages(MSG_CODE_START);
        timeHandler.removeMessages(MSG_CODE_END);
        timeHandler.removeMessages(MSG_CODE_SAVE);
    ...
    private void showFaceBgForFlash() {
        ...
        if (mScanRing != null) {
            mScanRing.setHoleMaskEnabled(false);
            mScanRing.setVisible(true);
        }
    ...
    private void openCameraAsync(final SurfaceHolder surfaceHolder, final boolean light) {
        if (mCameraOpenInFlight) {
            return;
        }
        mCameraOpenInFlight = true;
        new Thread(() -> {
            try {
                if (light) {
                    CameraManagerLight.get(this).openDriver(surfaceHolder, cameraDirection);
                    // 首次进页必须先 AliveInit 再 startPreview；后台返回时 Init 已完成，
                    // 这里立刻开预览，否则 continueAfterCameraOpenLight 会跳过 startPreview 导致黑屏
                    if (isAeyeLightInit && AEyeLightAlive.getInstance().isInitialized()) {
                        CameraManagerLight.get(this).startPreview();
                    }
                } else {
            ...
            if (light) {
                runOnUiThread(() -> {
                    mCameraOpenInFlight = false;
                    continueAfterCameraOpenLight();
                });
            } else {
                runOnUiThread(() -> {
                    mCameraOpenInFlight = false;
                    continueAfterCameraOpenMotion();
                });
            }
    ...
    /** 相机重新打开后：后台返回则直接开检（倒计时从头开始），否则走进页准备。 */
    private void resumeRecogAfterCameraOpen() {
        if (mResumeFromBackground) {
            mResumeFromBackground = false;
            boolean keepAction = mPausedWhileInAction;
            // 后台返回一律保留当前动作/序列，不要重算口令或回到「正视摄像头」
            startRecog(true);
            if (keepAction) {
                restoreCurrentActionAfterResume();
            }
            mPausedWhileInAction = false;
            return;
        }
        prepareRecog();
    }
    ...
    @Override
    /** 进后台：立刻停预览、关相机、停解码。不结束核验（切走再回来可继续）。 */
    ...
    protected void onStop() {
        super.onStop();
        // WakeLock 与 onStart 配对：部分遮挡只会 onPause、不会 onStop，避免弹窗时提前灭屏。
        releaseWakeLockQuietly();
    }

    /** 进后台：停掉动作/无人脸/多人/环境光倒计时，回来后按当前画面从头计。 */
    private void pauseLivenessCountdowns() {
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return;
        }
        mResumeFromBackground = true;
        mPausedWhileInAction = mGuideOvalState == 1 && shouldShowActionCountdown();
        stopTimer();
        mCountdown.clearActionRemain();
        cancelNoFaceFailTimer();
        cancelMultiFaceFailTimer();
        cancelLightAbnormalTimer(false);
        // 动作进行中：保留入框态，回来后继续当前动作；否则清零，让检测重新判定
        if (!mPausedWhileInAction) {
            mGuideOvalState = 0;
        }
        pauseLightFlashSequence();
    }

    /** 后台返回：立刻恢复当前动作文案，并从头再计该动作的 10s。 */
    private void restoreCurrentActionAfterResume() {
        mCountdown.setFrozen(false);
        mDisplayedGuideState = 1;
        mGuideOvalState = 1;
        restoreInFrameHint();
        if (shouldShowActionCountdown()) {
            restartTimer((int) AEFacePack.getInstance().getMotionTime(), FaceCountdownController.ACTION);
        }
    }

    private void armLightCheckGrace() {
        mLightCheckAfterElapsedMs = SystemClock.elapsedRealtime() + LIGHT_CHECK_GRACE_MS;
    }

    /** 预览刚打开时曝光未稳，这段时间内不提示过暗/过亮 */
    private boolean shouldAcceptLightAbnormal() {
        return SystemClock.elapsedRealtime() >= mLightCheckAfterElapsedMs;
    }

    private boolean shouldKeepCameraOff() {
        return mUsbDebugBlocked || mFaceVerifying || mInPlaceFailUi || mInPlaceSuccessUi
                || mFinish || m_hasFinishReturn || isFinishing();
    }

    private void releaseWakeLockQuietly() {
    ...
    public void showQualityHint(final int quality, final boolean voice) {
        ...
        switch (quality) {
            ...
            case QUALITY_OUT:
                ...
                // 文案走 notifyOutOfFrameHint / commitGuideUi 防抖，避免与「请靠近一点」每帧对打
                break;
            case QUALITY_SIDE:
        ...
        final int id = mId;
        final int playVoiceId = voiceId;
        final int qualityType = quality;
        if (textId != id) {
            textId = id;
            if (id == 0) {
                if (mQualityVoiceRunnable != null) {
            ...
            mQualityVoiceRunnable = () -> {
                mQualityVoiceRunnable = null;
                if (!voiceTriggle || playVoiceId == 0 || !voice) {
                    return;
                }
                // 检测抖动：脸已回框则不播「请将脸移入框内」，避免口型动作文案配出框语音
                if ((qualityType == QUALITY_OUT || qualityType == AEFaceQuality.QUALITY_FAR)
                        && mFaceOK > 0) {
                    return;
                }
                if (qualityType == QUALITY_OUT || qualityType == AEFaceQuality.QUALITY_FAR) {
                    AudioUtils.playVoice(getApplication(), playVoiceId);
                } else {
                    AudioUtils.playVoiceIdle(getApplication(), playVoiceId);
                }
            };
            mUIHandler.postDelayed(mQualityVoiceRunnable, 250);
        }
    ...
    private void applyCheckHintText(int textResId, int hintColorKind) {
        ...
        if (mWaitingMultiFace
                && (textResId == R.string.aeye_quality_out || textResId == R.string.aeye_face_far)) {
            return;
        }
        if (mWaitingLightAbnormal
                && (textResId == R.string.aeye_quality_out || textResId == R.string.aeye_face_far)) {
        ...
        if (shouldHoldInFrameHint()
                && textResId != R.string.aeye_quality_out
                && textResId != R.string.aeye_face_far) {
            return;
        }
        // 出框 UI 未提交前不刷动作口令，避免和「移入框内 / 靠近一点」对打
        if (mDisplayedGuideState != 1
                && textResId != R.string.aeye_quality_out
                && textResId != R.string.aeye_face_far) {
            return;
        }
        if ((textResId == R.string.aeye_quality_out || textResId == R.string.aeye_face_far)
                && shouldHoldOutHintRes(textResId)) {
            return;
        }
        if (isUpAndroid6) {
            ...
            if (textResId == R.string.aeye_quality_out) {
                mDisplayedCheckHintKey = "aeye_quality_out";
                mOutHintAppliedAt = SystemClock.elapsedRealtime();
            } else if (textResId == R.string.aeye_face_far) {
                mDisplayedCheckHintKey = "face_far";
                mOutHintAppliedAt = SystemClock.elapsedRealtime();
            } else {
                mDisplayedCheckHintKey = null;
            }
            textId = textResId;
            cancelPendingOutVoiceIfPoseHint(textResId);
        } else {
            showShortToast(getString(textResId));
        }
    }

    private void cancelPendingOutVoiceIfPoseHint(int textResId) {
        if (textResId == R.string.aeye_quality_out || textResId == R.string.aeye_face_far) {
            return;
        }
        if (mQualityVoiceRunnable != null) {
    ...
    private void showFaceStatus(final boolean face, final boolean voice) {
        ...
        if (face) {
            // 纯炫彩回框不走 showAlivePose，keep_face 也不会改 textId，
            // 不重置的话下次 QUALITY_OUT 会被当成重复文案而不播报
            if (textId == R.string.aeye_quality_out || textId == R.string.aeye_face_far) {
                textId = 0;
            }
            if (isDecode) {
                mUIHandler.sendEmptyMessage(UI_MSG_TVENVHINT_HIDE);
            }
            // 口令/出框文案由 commitGuideUi 防抖后再切，避免边缘抖动闪字
        } else if (mPendingOutHintKey == null) {
            mPendingOutHintKey = "aeye_quality_out";
        }
    }
    ...
    private void faceAppear() {
        if (AEFacePack.getInstance().isFaceAppearStartMode() && !faceDisplayed) {
            ...
            // 10s 只由 onFaceEnteredGuide 在入框确认后启动，避免 QUALITY_OK 先闪 10s
        }
        faceDisplayed = true;
    }
    ...
    public void showFaceOut(final boolean inRange) {
        showFaceOut(inRange, true);
    }

    public void showFaceOut(final boolean inRange, final boolean playVoice) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(() -> showFaceOut(inRange, playVoice));
            return;
        }
        if (inRange) {
            mLightSeqResetDone = false;
        ...
        if ((inRange && mFaceOK < 0) ||
            ...
            showFaceStatus(inRange, playVoice);
            if (isLightAliveMode()) {
                // 炫彩 / 动作+炫彩：脸不在圆形框内不转蓝弧，由检测线程确认入框后再开
                if (!inRange) {
            ...
            // 纯炫彩 / 已进闪光：入框与否由 notifyGuideOvalForTimeout 驱动。
            // 这里若因「检测到人脸」走 inRange=true，会误取消 20 秒并挡住出框文案。
            boolean ovalManagedSeparately = aliveMode == AEFaceParam.ALIVEMODE_LIGHT
                    || mLightFlashStarted;
            if (!ovalManagedSeparately) {
                if (inRange) {
                    onFaceEnteredGuide();
                } else {
                    onFaceLeftGuide();
    ...
    /** 炫彩 / 动作+炫彩：仅人脸在圆形引导框内时启动旋转蓝弧。 */
    public void setLightScanArcEnabled(boolean inGuideFrame) {
        if (!isLightAliveMode()) {
            return;
        }
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(() -> setLightScanArcEnabled(inGuideFrame));
            return;
        }
        updateScanRingForFace(inGuideFrame);
    }
    ...
    public void setMotionAliveSuc() {
        ...
        if (voiceTriggle) {
            new VoicePlayer(this).playVoice(R.raw.ding);
        }
    ...
    /**
     * 动作+炫彩：当前动作通过，切换到下一个动作的提示/语音。
     * 由解码线程回调，需切主线程。
     */
    public void onMotionLightPoseChanged(int poseId, boolean playVoice) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(() -> onMotionLightPoseChanged(poseId, playVoice));
            return;
        }
        setPose(poseId);
        if (playVoice && voiceTriggle) {
            new VoicePlayer(this).playVoice(R.raw.ding);
            mPlayedPoseVoiceId = poseId;
        }
        showAlivePose(poseId, playVoice, false);
        if (shouldShowActionCountdown() && mFaceOK > 0) {
            restartTimer(AEFacePack.getInstance().getMotionTime());
        }
    }
     ...
     * 距离临界时需稳定一段时间再切，避免 10s 与「请靠近一点」来回跳。
     */
    public void showFaceTooFar() {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(this::showFaceTooFar);
            return;
        }
        if ("face_far".equals(mDisplayedCheckHintKey) && mDisplayedGuideState < 0) {
            // 已在出框/无人脸流程时，不要把 pending 重新钉成「靠近一点」
            if (!"aeye_quality_out".equals(mPendingOutHintKey)
                    && !"quality_out".equals(mPendingOutHintKey)) {
                mPendingOutHintKey = "face_far";
            }
            return;
        }
        if (mPendingFarUiRunnable != null) {
            return;
        }
        mPendingFarUiRunnable = this::applyFaceTooFarNow;
        if (mUIHandler != null) {
            mUIHandler.postDelayed(mPendingFarUiRunnable, FAR_UI_HOLD_MS);
        } else {
            applyFaceTooFarNow();
        }
    }

    private void cancelPendingFarUi() {
        if (mPendingFarUiRunnable != null && mUIHandler != null) {
            mUIHandler.removeCallbacks(mPendingFarUiRunnable);
        }
        mPendingFarUiRunnable = null;
    }

    private void applyFaceTooFarNow() {
        mPendingFarUiRunnable = null;
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return;
        }
        ...
        if (mFaceOK > 0 || mFaceOK == 0) {
            mFaceOK = -1;
            sFaceOK = mFaceOK;
            updateScanRingForFace(false);
            Message statusMsg = Message.obtain();
            statusMsg.obj = false;
            statusMsg.what = UI_MSG_FACESTATUS;
            mUIHandler.sendMessage(statusMsg);
        }
        if (mQualityVoiceRunnable != null) {
            mUIHandler.removeCallbacks(mQualityVoiceRunnable);
            mQualityVoiceRunnable = null;
    ...
    public void showTipAfterHasFace() {
        ...
        int curPose = lightHandler != null ? lightHandler.getCurPos() : pose;
        int hintResId = resolvePoseHintTextId(curPose);
        // 与纯动作一致：同一动作只播一次口令，出框/过远再回框只恢复文案
        boolean playVoice = voiceTriggle && hintResId != 0 && mPlayedPoseVoiceId != curPose;
        if (playVoice) {
            mPlayedPoseVoiceId = curPose;
            textId = hintResId;
        }
        showAlivePose(curPose, playVoice, false);
    }

    /**
     * 连续 {@link #NO_FACE_FAIL_DELAY_MS} 未在框内则验证失败，页面显示 20s 倒计时；
     * 期间若重新入框则 {@link #cancelNoFaceFailTimer()}。
     * 第 15 项：首次进入检测从未入框；第 23 项：曾经入框后再出框。
     * 倒计时数字与「请将脸移入框内」在 {@link #commitGuideUi} 一并展示，避免先进页只出 20s。
     */
    private void scheduleNoFaceFailAfterDelay() {
        startNoFaceCountdown();
    }

    private void startNoFaceCountdown() {
        // 出框文案走 Handler 消息，倒计时必须排在它后面，避免先出 20s 再出「请将脸移入框内」
        if (mUIHandler != null) {
            mUIHandler.post(this::startNoFaceCountdownNow);
            return;
        }
        startNoFaceCountdownNow();
    }

    private void startNoFaceCountdownNow() {
        if (mGuideOvalState == 1 || mWaitingMultiFace || mWaitingLightAbnormal) {
            return;
        }
        if (!shouldUseOutOfFrameFail()) {
            return;
        ...
        if (mCountdown.isNoFace()) {
            return;
        }
        // 出框文案尚未提交时先冻住并藏掉数字，等 commitGuideUi 与「请将脸移入框内」一起出
        if (mDisplayedGuideState != -1) {
            mCountdown.setFrozen(true);
            hideActionCountdown();
        }
        ...
        int remainSec = (int) Math.ceil((NO_FACE_FAIL_DELAY_MS - elapsed) / 1000.0);
        if (remainSec < 1) {
            remainSec = 1;
        }
        if (remainSec > NO_FACE_FAIL_SEC) {
            remainSec = NO_FACE_FAIL_SEC;
        }
        restartTimer(remainSec, FaceCountdownController.NO_FACE);
    }
    ...
    private void cancelNoFaceFailTimer() {
        ...
        if (mCountdown != null) {
            mCountdown.cancelKind(FaceCountdownController.NO_FACE);
        }
    }

    /**
     * 第 16 项：框内多人。连续 20s 仍多人则失败；回到单脸/无人脸则取消。
     * 解码线程可调用；非多人且当前未在等待时不切主线程。
     */
    public void notifyMultiFace(boolean many) {
        if (!many && !mWaitingMultiFace) {
            return;
        }
        if (Looper.myLooper() != Looper.getMainLooper()) {
            final boolean manyFaces = many;
            runOnUiThread(() -> notifyMultiFace(manyFaces));
            return;
        }
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return;
    ...
    private void onMultiFaceDetected() {
        if (isLightAliveMode()) {
            pauseLightFlashSequence();
            mLightSeqResetDone = true;
        }
        ...
        cancelNoFaceFailTimer();
        cancelLightAbnormalTimer(false);
        if (mMultiFaceSinceElapsedMs < 0) {
            mMultiFaceSinceElapsedMs = SystemClock.elapsedRealtime();
        }
        mWaitingMultiFace = true;
        showHint("more_people", HINT_COLOR_THEME);
        startMultiFaceCountdown();
    }

    private void startMultiFaceCountdown() {
        if (mUIHandler != null) {
            mUIHandler.post(this::startMultiFaceCountdownNow);
            return;
        }
        startMultiFaceCountdownNow();
    }

    private void startMultiFaceCountdownNow() {
        if (!shouldUseOutOfFrameFail()) {
            return;
        }
        if (mInPlaceFailUi || mInPlaceSuccessUi) {
        ...
        if (mCountdown.isMultiFace()) {
            return;
        }
        long now = SystemClock.elapsedRealtime();
        if (mMultiFaceSinceElapsedMs < 0) {
            mMultiFaceSinceElapsedMs = now;
        }
        mWaitingMultiFace = true;
        long elapsed = now - mMultiFaceSinceElapsedMs;
        if (elapsed >= NO_FACE_FAIL_DELAY_MS) {
            failByMultiFace();
            return;
        }
        ...
        restartTimer(remainSec, FaceCountdownController.MULTI_FACE);
    }

    private void cancelMultiFaceFailTimer() {
        mWaitingMultiFace = false;
        mMultiFaceSinceElapsedMs = -1L;
        if (mCountdown != null) {
            mCountdown.cancelKind(FaceCountdownController.MULTI_FACE);
        }
    }

    private void failByMultiFace() {
        cancelMultiFaceFailTimer();
        cancelNoFaceFailTimer();
        cancelLightAbnormalTimer(false);
        noteLivenessFailForQrRecord();
        showInPlaceFailUi(false, getString(R.string.face_fail_multi_face_detail),
                FaceUniResultCodes.RESULT_MULTI_FACE);
    }

    /**
     * 第 17/18 项：环境光过暗/过亮。连续 20s 未调整则失败；恢复正常则取消。
     * {@code quality} 为 {@link AEFaceQuality#QUALITY_DARK} / {@link AEFaceQuality#QUALITY_BRIGHT}；
     * 其它值视为光线已正常。
     */
    public void notifyLightAbnormal(int quality) {
        boolean abnormal = quality == AEFaceQuality.QUALITY_DARK
                || quality == AEFaceQuality.QUALITY_BRIGHT;
        if (!abnormal && !mWaitingLightAbnormal) {
            return;
        }
        if (Looper.myLooper() != Looper.getMainLooper()) {
            final int q = quality;
            runOnUiThread(() -> notifyLightAbnormal(q));
            return;
        }
        if (mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn) {
            return;
        }
        // 纯炫彩、以及已进入色光：屏幕亮度会变，不做 17/18。动作+炫彩的动作阶段仍要判定。
        if (aliveMode == AEFaceParam.ALIVEMODE_LIGHT || mLightFlashStarted) {
            cancelLightAbnormalTimer(false);
            return;
        }
        // 出框 / 过远 / 入框防抖期间，边缘脸很容易被判过暗，不当成环境光
        if (mGuideOvalState != 1 || mDisplayedGuideState != 1) {
            if (mWaitingLightAbnormal) {
                cancelLightAbnormalTimer(false);
            }
            return;
        ...
        if (!shouldAcceptLightAbnormal()) {
            return;
        }
        onLightAbnormal(quality);
    }

    /** 光线恢复正常（解码线程可调用） */
    public void notifyLightNormal() {
        if (!mWaitingLightAbnormal) {
            return;
        }
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(this::notifyLightNormal);
            return;
        }
        cancelLightAbnormalTimer(true);
    }

    private void onLightAbnormal(int quality) {
        if (mWaitingMultiFace) {
            return;
        }
        if (mWaitingLightAbnormal && mLightAbnormalQuality == quality) {
            startLightAbnormalCountdown();
            return;
        }
        if (isLightAliveMode()) {
            pauseLightFlashSequence();
        ...
        mCountdown.stashActionRemain();
        cancelNoFaceFailTimer();
        if (mLightAbnormalQuality != quality) {
            mLightAbnormalSinceElapsedMs = SystemClock.elapsedRealtime();
        }
        if (mLightAbnormalSinceElapsedMs < 0) {
            mLightAbnormalSinceElapsedMs = SystemClock.elapsedRealtime();
        }
        mLightAbnormalQuality = quality;
        mWaitingLightAbnormal = true;
        showHint(lightAbnormalHintKey(), HINT_COLOR_THEME);
        if (voiceTriggle) {
            int voiceRes = quality == AEFaceQuality.QUALITY_BRIGHT
                    ? R.raw.aeye_quality_bright : R.raw.aeye_quality_dim;
            AudioUtils.playVoiceIdle(getApplication(), voiceRes);
        }
        startLightAbnormalCountdown();
    }

    private String lightAbnormalHintKey() {
        return mLightAbnormalQuality == AEFaceQuality.QUALITY_BRIGHT ? "light_bright" : "light_dark";
    }

    private void startLightAbnormalCountdown() {
        if (mUIHandler != null) {
            mUIHandler.post(this::startLightAbnormalCountdownNow);
            return;
        }
        startLightAbnormalCountdownNow();
    }

    private void startLightAbnormalCountdownNow() {
        if (!shouldUseOutOfFrameFail()) {
            return;
        }
        if (mInPlaceFailUi || mInPlaceSuccessUi || mWaitingMultiFace) {
            return;
        }
        if (mCountdown.isLight()) {
            return;
        }
        long now = SystemClock.elapsedRealtime();
        if (mLightAbnormalSinceElapsedMs < 0) {
            mLightAbnormalSinceElapsedMs = now;
        }
        mWaitingLightAbnormal = true;
        long elapsed = now - mLightAbnormalSinceElapsedMs;
        if (elapsed >= NO_FACE_FAIL_DELAY_MS) {
            failByLightAbnormal();
            return;
        }
        ...
        restartTimer(remainSec, FaceCountdownController.LIGHT);
    }

    /**
     * @param resumeAction 光线恢复时，若人脸仍在框内则接着跑动作 10s
     */
    private void cancelLightAbnormalTimer(boolean resumeAction) {
        boolean wasWaiting = mWaitingLightAbnormal;
        mWaitingLightAbnormal = false;
        mLightAbnormalQuality = 0;
        mLightAbnormalSinceElapsedMs = -1L;
        if (mCountdown != null) {
            mCountdown.cancelKind(FaceCountdownController.LIGHT);
        }
        if (resumeAction && wasWaiting && mGuideOvalState == 1 && shouldShowActionCountdown()) {
            int sec = mCountdown.getActionRemainSec() > 0
                    ? mCountdown.getActionRemainSec()
                    : (int) AEFacePack.getInstance().getMotionTime();
            mCountdown.clearActionRemain();
    ...
    private void failByLightAbnormal() {
        int quality = mLightAbnormalQuality;
        cancelLightAbnormalTimer(false);
        cancelNoFaceFailTimer();
        cancelMultiFaceFailTimer();
        noteLivenessFailForQrRecord();
        int detailRes = quality == AEFaceQuality.QUALITY_BRIGHT
                ? R.string.face_fail_light_bright_detail
                : R.string.face_fail_light_dark_detail;
        String exceptionCode = quality == AEFaceQuality.QUALITY_BRIGHT
                ? FaceUniResultCodes.RESULT_LIGHT_BRIGHT
                : FaceUniResultCodes.RESULT_LIGHT_DARK;
        showInPlaceFailUi(false, getString(detailRes), exceptionCode);
    }

    /** 第 12 项首次无人脸 / 第 17 项后续出框，失败说明不同 */
    private String resolveLoseFaceFailDetail() {
        if (faceDisplayed) {
            return getString(R.string.face_fail_action_no_face_detail);
        }
        return getString(R.string.face_fail_no_face_detail);
    }

    /** 第 12 项 {@code 0412002} / 第 17 项 {@code 0414009} */
    private String resolveLoseFaceFailCode() {
        return faceDisplayed
                ? FaceUniResultCodes.RESULT_ACTION_OUT_OF_FRAME
                : FaceUniResultCodes.RESULT_NO_FACE_TIMEOUT;
    }
    ...
    /**
     * 第 22 项：单动作超时（默认 10s），内联失败文案与第 23 项相同。
     */
    ...
    public void resetData() {
        ...
        sFaceOK = 0;
        mGuideOvalState = 0;
        mDisplayedGuideState = 0;
        if (mCountdown != null) {
            mCountdown.resetSessionFlags();
        }
        mPendingOutHintKey = null;
        mPendingGuideUiIn = null;
        cancelGuideUiCommit();
    ...
    public void finishActivityByOther(final int code, final String reason) {
        ...
        stopRingProgress();
        // 其它错误结束流程：不上报 status=2；避免 onDestroy 误报任务取消
    ...
    public void finishActivityBySuccessful() {
        ...
        // 动作活体本地通过即报结束日志，不绑定后续 faceIdent 提交结果
        ...
        buildLivenessJsonAsync(json -> {
            ...
            FaceVerifyManager.submit(json, new FaceVerifyManager.Callback() {
                ...
                public void onFailed(String message) {
                    onFailed(message, null);
                }

                @Override
                public void onFailed(String message, String backendErrorCode) {
                    if (isFinishing()) {
                        return;
                    }
                    runOnUiThread(() -> onFaceVerifyFailed(message, backendErrorCode));
                }
            });
        });
    }
    ...
    private void submitColorVerify(final String lightJson, final Bitmap cuesBitmap) {
        ...
        if (facePics == null || facePics.length() == 0) {
            runOnUiThread(() -> {
                ...
                showInPlaceFailUi(false, "炫彩人脸图组装失败", true,
                        FaceUniResultCodes.RESULT_SERVICE_ERROR);
            });
            return;
        }
        ...
        scheduleVerifyTimeout(faceVerifyUiTimeoutMs());
     ...
     * 后续 {@link ScanRingAnimator#animateToSuccess()} 的「核验通过」蓝弧填充/绿环无从显示。
     */
    private void startVerifyingRingArc() {
        if (mInPlaceSuccessUi || mInPlaceFailUi || mScanRing == null) {
            return;
        }
        mScanRing.prepareVerifying();
    }

    /** 「人脸核验中」UI 兜底超时（faceIdent + queryVerifyResult 轮询） */
    private long faceVerifyUiTimeoutMs() {
        return FaceVerifyManager.recommendedUiTimeoutMs();
    }

    /** 核验请求挂死时，超时切到失败 UI，避免一直停在「人脸核验中」 */
    private void scheduleVerifyTimeout(long timeoutMs) {
        cancelVerifyTimeout();
        final long delay = timeoutMs > 0 ? timeoutMs : faceVerifyUiTimeoutMs();
    ...
    private void onFaceVerifyPassed() {
        if (mInPlaceFailUi || mInPlaceSuccessUi || isFinishing()) {
            return;
        }
        cancelVerifyTimeout();
        cancelGuideUiCommit();
        ...
        hideSubmitOverlay();
        // 查询核验结果已确认通过；不报 status=5（产品约定由后台/业务二次确认）
        // 结束日志已在本地活体通过时上报，此处不再按提交结果重报。
        mQrRecordFinalized = true;
        showInPlaceVerifySuccess();
        startSuccessAutoFinishCountdown();
    }

    private void onFaceVerifyFailed(String message) {
        onFaceVerifyFailed(message, null);
    }

    private void onFaceVerifyFailed(String message, String backendErrorCode) {
        cancelVerifyTimeout();
        setFaceVerifying(false);
        String failReason = TextUtils.isEmpty(message)
                ? getString(R.string.face_verify_submit_server_error)
                : message;
        markQrRecordNotPass();
        if (TextUtils.isEmpty(backendErrorCode)) {
            backendErrorCode = FaceUniResultCodes.submitSceneCode(failReason);
        }
        AEFacePack.getInstance().setPendingAuthStatusError(backendErrorCode, failReason);
    ...
    private void startSuccessAutoFinishCountdown() {
        if (mSuccessFinishRunnable != null) {
            mUIHandler.removeCallbacks(mSuccessFinishRunnable);
        }
        mSuccessRemainSec = SUCCESS_AUTO_FINISH_SEC;
        bindSuccessJumpHint(mSuccessRemainSec);
        mSuccessFinishRunnable = new Runnable() {
            @Override
            public void run() {
                mSuccessRemainSec--;
                if (mSuccessRemainSec <= 0) {
                    mSuccessFinishRunnable = null;
        ...
        final Runnable tick = mSuccessFinishRunnable;
        // 等结果文案完成布局后再开始减秒，避免第一帧还没画出来就变成 2S
        View anchor = tvFailDetail != null ? tvFailDetail : tvCheckHint;
        if (anchor != null) {
            anchor.post(() -> {
                if (mSuccessFinishRunnable != tick || isFinishing()) {
                    return;
                }
                mUIHandler.postDelayed(tick, 1000);
            });
        } else {
            mUIHandler.postDelayed(tick, 1000);
        }
    ...
    private void bindSuccessJumpHint(int seconds) {
        if (tvFailDetail == null) {
            return;
        }
        String full = getString(R.string.face_verify_success_auto_jump, seconds);
        ...
        tvFailDetail.setText(sp);
    }

    /** 出结果后隐藏取景/扫描环/闪光，只留结果文案 */
    private void hidePreviewForResult() {
        View panel = findViewById(R.id.face_preview_panel);
        if (panel != null) {
            panel.setVisibility(View.GONE);
        }
        View slot = findViewById(R.id.face_scan_ring_slot);
        if (slot != null) {
            slot.setVisibility(View.GONE);
        }
        if (mScanRing != null) {
            mScanRing.setVisible(false);
        }
        if (faceBgView != null) {
            hideFaceBgFlash();
        }
        ...
        collapseTipColumnForResult();
        // 失败/成功页把 SurfaceView 设为 GONE 后 Surface 会被销毁。
        // 若仍占着旧 Surface，隔很久再点「重新核验」startPreview 会黑屏。
        releaseCameraAfterPreviewHidden();
    }

    private void releaseCameraAfterPreviewHidden() {
        try {
            if (isLightAliveMode()) {
                CameraManagerLight.get(this).stopPreview();
                CameraManagerLight.get(this).closeDriver();
    ...
    private void collapseTipColumnForResult() {
        ...
        if (mCountdown != null) {
            mCountdown.hideForResult();
        }
    }

    private void restoreTipColumnAfterRetry() {
        if (faceTipColumn != null) {
    ...
    public void finishActivityByFail() {
        ...
        noteLivenessFailForQrRecord();
        showInPlaceFailUi(false, getString(R.string.face_fail_liveness_algorithm_detail),
                FaceUniResultCodes.RESULT_LIVENESS_FAIL);
    }
    ...
    public void showInPlaceFailUi(boolean timeout, String customDetail) {
        showInPlaceFailUi(timeout, customDetail, false, null);
    }

    /** 本地场景失败：点返回时 {@code onFinish} 回传 {@code exceptionCode}。 */
    public void showInPlaceFailUi(boolean timeout, String customDetail, String exceptionCode) {
        showInPlaceFailUi(timeout, customDetail, false, exceptionCode);
    }

    /**
     * @param submitFailure true=faceIdent 提交阶段失败（标题「核验失败」）；false=活体等失败（标题「验证失败」）
     */
    public void showInPlaceFailUi(boolean timeout, String customDetail, boolean submitFailure) {
        showInPlaceFailUi(timeout, customDetail, submitFailure, null);
    }

    public void showInPlaceFailUi(boolean timeout, String customDetail, boolean submitFailure,
                                  String exceptionCode) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            runOnUiThread(() -> showInPlaceFailUi(timeout, customDetail, submitFailure, exceptionCode));
            return;
        }
        if (mInPlaceFailUi || mInPlaceSuccessUi) {
            return;
        ...
        if (!TextUtils.isEmpty(customDetail)) {
            ...
            detail = !TextUtils.isEmpty(pending)
                    ...
                    ? R.string.face_verify_submit_server_error
                    : R.string.face_fail_no_face_detail);
        }
        if (!timeout) {
            persistSceneExceptionCode(exceptionCode, detail);
        }
    ...
    /** 点「返回」时 onFinish 使用的场景码；第 21/22 项后台码已写入则不覆盖。 */
    private void persistSceneExceptionCode(String exceptionCode, String detail) {
        AEFacePack pack = AEFacePack.getInstance();
        if (!TextUtils.isEmpty(pack.getPendingBackendErrorCode())) {
            return;
        }
        if (!TextUtils.isEmpty(exceptionCode)) {
            pack.setPendingAuthStatusError(exceptionCode, detail);
        } else if (!TextUtils.isEmpty(detail)) {
            pack.setPendingFailDetail(detail);
        }
    }

    private void showFailActions() {
        if (faceFailActions != null) {
            faceFailActions.setVisibility(View.VISIBLE);
        }
        if (btnFailRetry != null) {
    ...
    private void hideFailActions() {
        if (faceFailActions != null) {
            faceFailActions.setVisibility(View.GONE);
        }
        if (btnFailRetry != null) {
    ...
    private void resetInPlaceFailUi() {
        ...
        showPreviewAfterFailRetry();
        restoreTipColumnAfterRetry();
        cancelNoFaceFailTimer();
        cancelMultiFaceFailTimer();
        cancelLightAbnormalTimer(false);
        faceDisplayed = false;
    ...
    private void onInPlaceFailRetry() {
        mFailRetryClickCount++;
        if (mFailRetryClickCount > MAX_FAIL_RETRY_CLICKS) {
            FaceNoticeDialog.show(this,
                    R.string.aeye_retry_too_many_title,
                    R.string.aeye_retry_too_many_message,
                    R.string.aeye_retry_too_many_confirm,
                    this::handleUserExit);
            return;
        }
        FaceVerifySession.resetEndLogSent();
        mQrRecordFinalized = false;
        AEFacePack.getInstance().setPendingFailDetail(null);
        QrRecordStatusManager.update(QrRecordStatus.VERIFYING);
        resetInPlaceFailUi();
        // 失败页会 GONE 掉 SurfaceView 并释放相机；不能只 startPreview。
        // 等 Surface 重建后走 initCamera（与 onResume 相同）。
        if (isLightAliveMode()) {
    ...
    /** 失败页重试：Surface 若已就绪立刻开相机，否则等 {@link #surfaceCreated}。 */
    private void reopenCameraAfterFailRetry() {
        if (isFinishing() || isDestroyed() || m_hasFinishReturn) {
            return;
        }
        mResumeFromBackground = false;
        ...
        if (surfaceView == null) {
            Log.e(TAG, "onInPlaceFailRetry: preview_view missing");
            return;
        }
        SurfaceHolder holder = surfaceView.getHolder();
        if (hasSurface || isSurfaceReady(holder)) {
            hasSurface = true;
            initCamera(holder);
        }
    }
    ...
    /**
     * 用户主动退出：失败结果页点「返回」回传当前场景码；核验中途退出才是用户取消。
     */
    ...
    private void exitAfterInPlaceFail() {
        ...
        // 结束日志兜底（会话内去重；活体失败已在出失败页时报过）+ 二维码终态兜底
        FaceVerifyLogManager.uploadVerifyEnd(getApplicationContext(), false,
        ...
        if (timeout) {
            ...
            buildLivenessJsonAsync(json -> {
                AEFaceCallbackHelper.dispatchFinish(listener, AEFacePack.ERROR_FAIL, json, failDetail, submitFail,
                        AEFacePack.getInstance().getPendingBackendErrorCode());
                finish();
            });
        }
    }

    private boolean isScanRingBlocked() {
        return mFinish || m_hasFinishReturn || mInPlaceSuccessUi || mInPlaceFailUi || mFaceVerifying;
    }
    ...
    /** 有人脸时启动蓝色旋转弧。纯动作无人脸延迟隐藏以免切动作停一帧；炫彩/动作+炫彩出框立即停。 */
    private void updateScanRingForFace(boolean hasFace) {
        if (isScanRingBlocked() || mScanRing == null) {
            return;
        }
        if (hasFace && mScanRing.isAvailable()) {
            cancelNoFaceFailTimer();
        }
        mScanRing.updateForFace(hasFace, isLightAliveMode());
    }
    ...
    private final class CountdownHost implements FaceCountdownController.Host {
        @Override
        public boolean isCountdownBlocked() {
            return mInPlaceFailUi || mInPlaceSuccessUi || mFaceVerifying || mFinish || m_hasFinishReturn;
        }

        @Override
        public boolean shouldShowActionCountdown() {
            return RecognizeActivity.this.shouldShowActionCountdown();
        }

        @Override
        public boolean canShowCountdownNumber() {
            return RecognizeActivity.this.canShowCountdownNumber();
        }

        @Override
        public boolean shouldSkipActionTimer() {
            if (aliveMode == AEFaceParam.ALIVEMODE_LIGHT) {
                return true;
            }
            return aliveMode == AEFaceParam.ALIVEMODE_MOTION_LIGHT && mLightFlashStarted;
        }

        @Override
        public int displayedGuideState() {
            return mDisplayedGuideState;
        }

        @Override
        public Handler timerHandler() {
            return mUIHandler != null ? mUIHandler : handler;
        }
        ...
        @Override
        public void onCountdownFinished(int kind) {
            handleCountdownFinished(kind);
        }
    }

    private void handleCountdownFinished(int kind) {
        if (m_hasFinishReturn) {
            return;
        }
        if (kind == FaceCountdownController.NO_FACE) {
            noteLivenessFailForQrRecord();
            showInPlaceFailUi(false, resolveLoseFaceFailDetail(), resolveLoseFaceFailCode());
            return;
        }
        if (kind == FaceCountdownController.MULTI_FACE) {
            failByMultiFace();
            return;
        }
        if (kind == FaceCountdownController.LIGHT) {
            failByLightAbnormal();
            return;
        }
        if (kind == FaceCountdownController.ACTION) {
            noteLivenessFailForQrRecord();
            showInPlaceFailUi(false, getString(R.string.face_fail_action_no_face_detail),
                    FaceUniResultCodes.RESULT_ACTION_TIMEOUT);
            return;
        ...
        if (AEFacePack.getInstance().isModelAllSide() &&
            ...
            if (side <= IDConstants.SIDE_MAX) {
                ...
                restartTimer(AEFacePack.getInstance().getMotionTime());
            } else if (IDConstants.SIDE_NUM == PictureManagerUtils.getPictureManager().getCurNum()) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK && mFaceVerifying) {
            return true;
        }
    ...
    /** 后台比对等未通过：上报 status=4，不传 failedType */
    ...
    private void markQrRecordNotPass(String failedType) {
        ...
        mQrRecordFinalized = true;
        // queryVerifyResult 轮询结束后仍未拿到 4/5 已上报 status=2，勿再覆盖为 4
        if (QrRecordStatusManager.isTerminalReported()) {
            return;
        }
        QrRecordStatusManager.update(QrRecordStatus.NOT_PASS, failedType);
    }

    /** 用户返回 / 系统返回：status=6，必传 failedType=6（已取消） */
    private void markQrRecordCancelled() {
        if (mQrRecordFinalized) {
            return;
        }
        mQrRecordFinalized = true;
    ...
    /**
     * 累计动作活体未通过次数；满 3 次上报 {@code status=4} + {@code failedType=1}。
     * 超时、后台比对失败不计入。
     */
